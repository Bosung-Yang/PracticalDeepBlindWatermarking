...
def main():
    ...
    checkpoint = torch.load('299epoch.pyt')
    ...
    for image in images:
        ...
        decoded_string1 = ''.join(str(int(x)) for x in decoded_rounded[0])
        xor = 0

        decoded_messages = model.ED.decoder(img2_tensor)
        decoded_rounded = decoded_messages.detach().cpu().numpy().round().clip(0, 1)
        decoded_string2 = ''.join(str(int(x)) for x in decoded_rounded[0])
        xor = 0

        decoded_messages = model.ED.decoder(img3_tensor)
        decoded_rounded = decoded_messages.detach().cpu().numpy().round().clip(0, 1)
        decoded_string3 = ''.join(str(int(x)) for x in decoded_rounded[0])
        xor = 0

        decoded_messages = model.ED.decoder(img4_tensor)
        decoded_rounded = decoded_messages.detach().cpu().numpy().round().clip(0, 1)
        decoded_string4 = ''.join(str(int(x)) for x in decoded_rounded[0])
        xor = 0

        decoded_string = ''
        for idx in range(30):
            if int(decoded_string1[idx]) + int(decoded_string2[idx]) + int(decoded_string3[idx]) + int(decoded_string4[idx]) >= 1:
                decoded_string += '1'
            else:
                decoded_string += '0'
        
        for idx in range(30):
            if decoded_string[idx] != message_string[idx]:
                xor+=1
        errors.append(xor/30)
    ...
    for image in images:
        ...
        decoded_string = ''
        for idx in range(30):
            if int(decoded_string1[idx]) + int(decoded_string2[idx]) + int(decoded_string3[idx]) + int(decoded_string4[idx]) > 2:
                decoded_string += '1'
            else:
                decoded_string += '0'
    ...
    for image in images:
        ...
        decoded_string = ''
        for idx in range(30):
            if int(decoded_string1[idx]) + int(decoded_string2[idx]) + int(decoded_string3[idx]) + int(decoded_string4[idx]) >= 3:
                decoded_string += '1'
            else:
                decoded_string += '0'
